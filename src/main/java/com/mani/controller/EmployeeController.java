...
public class EmployeeController {
    ...
    public void getEmployee(){
          Employee emp = new Employee("gbs05348","anbu","developer");
           employeeRepository.save(emp);
        Employee emp1 = new Employee("gbs05347","mani","developer");
        employeeRepository.save(emp1);
        System.out.println("insertion successfull");
    }

    @PostMapping("/uploadimage")
    public String saveEmployeeFile(@RequestParam("name") String name, @RequestParam("imageFile") MultipartFile file){
    ...
    @GetMapping("/image")
    public String getEmployeeImage(HttpServletResponse response, String id){
        GridFSFile file=gridFsTemplate.findOne(new Query(Criteria.where("metadata.name").is("mani").and("_id").is(id)));
        try {
    ...
    public String getEmployeeImages(HttpServletResponse response, String name){
        ...
        try(ZipOutputStream zipOutputStream = new ZipOutputStream(response.getOutputStream())){
            ...
            //    InputStream inputStream = gridFsTemplate.getResource(gridFSFile).getInputStream();
                ...
                FileCopyUtils.copy(gridFsTemplate.getResource(gridFSFile).getInputStream(), zipOutputStream);
